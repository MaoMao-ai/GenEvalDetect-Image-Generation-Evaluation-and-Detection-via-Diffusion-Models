...
class JoePennaDreamboothConfigSchemaV1:
    ...
    def validate_gpu_vram(self):
        ...
        required_vram = 15 * 1024 * 1024 * 1024  # 改成你自己的要求，比如 12GB
        if gpu_vram < required_vram:
            print(f"⚠️ Warning: Low VRAM ({convert_size(gpu_vram)} < {convert_size(required_vram)}). You may run into OOM issues.")
